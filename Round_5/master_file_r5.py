...
class Trader:
        ...
        for product in products:
            ...
            if price_range == 0:
                ...
                if current_vwap <= lower_bound:
                    ...
                    if Nsteps > 1 and upper_bound > lower_bound : 
            ...
            if delta != 0:
                ...
                price_to_use = 0
                if delta > 0: 
                    price_to_use = int(best_bid) if product =="SQUID_INK" else int((best_bid+1))
                   
                    orders_for_product.append(Order(product, price_to_use, delta))
                else:
                    price_to_use = int(best_ask) if product =="SQUID_INK" else int(( best_ask-1))
    ...
    def trade_9500(self, state, market_data, traderObject):
        ...
        threshold = 0
        if diff > threshold:  
            amount = market_data.sell_sum["VOLCANIC_ROCK_VOUCHER_9500"]
            amount = min(amount, sum(market_data.bid_volumes["VOLCANIC_ROCK_VOUCHER_9500"]))
            option_amount = amount
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        orders_9500 = self.trade_9500(state,market_data,traderObject)
