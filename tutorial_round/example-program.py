...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if len(order_depth.sell_orders) != 0:
                ...
                for ask_price, ask_volume in sorted_sells:
                    ...
                    if int(ask_price) < acceptable_price and max_buy_volume > 0:
                        # Limit the volume to our max allowed position
                        buy_volume = min(-ask_volume, max_buy_volume)
                        if buy_volume > 0:
            ...
            if len(order_depth.buy_orders) != 0:
                ...
                # Process all buy orders that are above our acceptable price
                for bid_price, bid_volume in sorted_buys:
                    ...
                    if int(bid_price) > acceptable_price and max_sell_volume > 0:
                        # Limit the volume to our max allowed position
                        sell_volume = min(bid_volume, max_sell_volume)
                        if sell_volume > 0:
    ...
    def calculate_acceptable_price(self, product, order_depth):
        ...
        default_price = 10000 if product == "RAINFOREST_RESIN" else 2000
        ...
        if product == "RAINFOREST_RESIN":
            ...
            if product in self.historical_prices and len(self.historical_prices[product]) >= 5:
                # Use the mean of historical prices with a small margin
                ...
                return max(buy_prices) * 1.01  # Add a small margin
            elif sell_prices:
                # If we only have sell orders, use the lowest sell price
                return min(sell_prices) * 0.99  # Subtract a small margin
            else:
                return default_price
                
        elif product == "KELP":
       
           # For stable RAINFOREST_RESIN, use a more conservative approach
            if product in self.historical_prices and len(self.historical_prices[product]) >= 5:
                # Use the mean of historical prices with a small margin
                hist_mean = mean(self.historical_prices[product])
                return hist_mean
                ...
                # If we only have buy orders, use the highest buy price
                return max(buy_prices) * 1.01  # Add a small margin
            elif sell_prices:
                # If we only have sell orders, use the lowest sell price
                return min(sell_prices) * 0.99  # Subtract a small margin
