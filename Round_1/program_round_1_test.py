    ...
    def run(self, state: TradingState):
        ...
        for product in state.order_depths:
            ...
            if len(order_depth.buy_orders) != 0 and len(order_depth.sell_orders) != 0 :
                best_bid = max(price * vol for price, vol in order_depth.buy_orders.items())
                best_ask = min(price * vol for price, vol in order_depth.sell_orders.items())
                mid = (best_bid+best_ask)/2

                if mid - acceptable_price > 0:
                    sell_volume = 1
                    if sell_volume > 0:
                        print(f"SELL {sell_volume}x {max(int(price) for price,vol in order_depth.buy_orders.items())}")
                        orders.append(Order(product, max(int(price) for price,vol in order_depth.buy_orders.items()), -sell_volume))
                        # Update our position tracking
                        self.positions[product] = self.positions.get(product, 0) - sell_volume


                if mid - acceptable_price < -0.5 : 
                    buy_volume = 1
                    
                    print(f"BUY {buy_volume}x {min(int(price) for price,vol in order_depth.sell_orders.items())}")
                    orders.append(Order(product, min(int(price) for price,vol in order_depth.sell_orders.items()), buy_volume))
                    # Update our position tracking
