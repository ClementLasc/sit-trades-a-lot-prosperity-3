...
PARAMS = {
    ...
    Product.SQUID_INK: {
        ...
        "sma_lookback": 5,     #5 best yet    
        "sma_default_price": 1973,
        # --- Market Taking Params ---
        "take_enabled": True, # Works well with sma
        "take_width": 5,       # 5 best yet
        # --- Market Making Params ---
        "make_enabled": True, # Works well with vwap
        "quote_volume": 3,  #3  
        "disregard_edge": 1, # 1
        "join_edge": 0, # 0
        "default_edge": 3,  # 3
       
    }, # Best : 1600 both market making and sma 
}
    ...
    def run(self, state: TradingState) -> tuple[dict[str, list[Order]], int, str]:
        ...
        for product in state.order_depths.keys():
            ...
            if params.get("make_enabled", True):
                 if product =="SQUID_INK" :
                     fair_value=self._fair_value_vwap(product, order_depth, params)
